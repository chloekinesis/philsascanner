...
# Pretty, boxed report -- shown in the terminal only, for the person
# running the script to read while it's on screen.
REPORT="
==========================================
...
# Form fields that strip newlines on paste would otherwise turn the boxed
# report above into one giant run-on wall of text. Using " | " as a
# separator instead of newlines means it reads the same clean way
# whether it lands in the form field, a CSV export, or Excel.
DISPLAYS_INFO_FLAT=$(echo "$DISPLAYS_INFO" | paste -sd "; " -)

FLAT_REPORT="Manufacturer: $MANUFACTURER | Product Family: $MODEL_NAME | Model Identifier: $MODEL_ID | Serial Number: $SERIAL_NUM | Motherboard: Apple Logic Board | System Type: $SYS_ARCH | CPU: $CHIP_NAME | CPU Cores: $CORES | CPU Threads: N/A (Apple Architecture) | CPU Gen / Series: Apple Silicon / Intel Mac | RAM: $RAM_GB GB | C: Drive Capacity: $STORAGE_TOTAL | C: Drive Free: $STORAGE_FREE | Physical Disk(s): $DISK_MODEL - $STORAGE_TOTAL | GPU: $GPU_INFO | DirectX: N/A (Uses Apple Metal API) | OS: $OS_NAME | OS Version: $OS_VER | OS Build: $OS_BUILD | Monitor Count: $MONITOR_COUNT | Monitor Info: $DISPLAYS_INFO_FLAT | TPM Present: N/A (Apple Secure Enclave) | TPM Version: Secure Enclave Processor | Firmware: Apple EFI / iBoot | Secure Boot: $SIP_STATUS (System Integrity Protection) | Webcam: $CAMERA | Microphone: $MIC | Speaker: $SPEAKER | Battery: $BATTERY_STATUS"

# Output the pretty version to terminal
echo "$REPORT"

# Copy the flat, form-friendly version to the Mac clipboard
echo "$FLAT_REPORT" | pbcopy
